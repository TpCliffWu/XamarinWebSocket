...
namespace TestApp
{
    public static class WebSocketExtensions
    {
        ...
        /// <returns></returns>
        public static  IEnumerable<string> WebSocketReadStrings(this ITcpSocketClient client, CancellationToken cancellationToken)
        {
            ...
            while (bytesRec != -1 && !cancellationToken.IsCancellationRequested && !gotEof)
            {
                ...
                bytesRec = client.ReadStream.Read(bytes, 0, bytes.Length);
                currData += Encoding.UTF8.GetString(bytes, 0, bytesRec);

       
                if (Regex.IsMatch(currData, "^GET", RegexOptions.IgnoreCase))
                {
                    ...
                    string swk = Regex.Match(currData, "Sec-WebSocket-Key: (.*)").Groups[1].Value.Trim();
